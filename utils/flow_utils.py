...
def vis_flow(flow, save_fig = False, save_dir = None):
	...
	u = u/(maxrad+eps)
	v = v/(maxrad+eps)
	img = computeColor(-v, -u) #TODO, change the direction according to my visualization. Not sure it is correct.
	out = img[:,:,[2,1,0]]
	if save_fig:
		imageio.imsave(save_dir, out)
	return out

import torch
if __name__ == '__main__':
	x = torch.ones((1, 2, 200, 200)) * -1
	save_flow(x, 'tmp/x.flo')

	flow = load_flow('tmp/x.flo')
	print(np.max(np.abs(x.numpy() - flow)))
	img = vis_flow(flow)
	import imageio
